...
const DEFAULT_RESIDENT_KEYWORDS = [
  ...
  'наш офис', 'мой офис', 'наш договор',
  // WiFi / Интернет
  'пароль от wifi', 'пароль от вайфая', 'код от переговорной',
  'код от кабинета', 'роутер', 'интернет пропал', 'интернет',
  // Поломки / Ремонт
  'проблема с', 'не работает', 'сломалось', 'сломан стул',
  'течет потолок', 'пожарная сигнализация', 'починить',
  'не работает принтер', 'кондиционер',
  // Доступ / Ключи
  'доступ по отпечатку', 'открыть кабинет', 'открыть переговорную',
  'карточка доступа', 'пропуск не работает', 'настроить палец',
  'добавить отпечаток', 'ключ',
  // Бытовое
  'распечатать', 'сканер', 'уборка', 'уборка в офисе',
  'душно в офисе', 'холодно в офисе', 'жарко в офисе',
  'кофе закончился', 'вода закончилась', 'пурифайер',
  'лампочку заменить', 'забрать мусор',
  // Документы
  'акт сверки', 'закрывающие документы', 'расторжение договора',
  // Приложение / ЛК
  'приложение коворкинга', 'личный кабинет', 'добавить сотрудника в приложение',
];
    ...
    switch (type) {
      case ContactType.CLIENT:
        return {
          ...
          additionalInstructions: 'Полный сервис. Помоги клиенту выбрать подходящее помещение и записаться на просмотр. НИКОГДА не раскрывай внутреннюю классификацию типа контакта клиенту.',
        };
      ...
      case ContactType.BROKER:
        return {
          ...
          ignoreMessages: false,
          additionalInstructions: '⚠️ КРИТИЧЕСКИ ВАЖНО: НИКОГДА не говори клиенту что он брокер/агент! ЗАПРЕЩЕНО писать "вы брокер", "понятно, вы агент", "раз вы брокер" и любые подобные фразы, раскрывающие внутреннюю классификацию. Общайся с ним так же вежливо, как с любым другим клиентом. Дай общую информацию об офисах и предложи связаться с менеджером для уточнения деталей сотрудничества. НЕ раскрывай что ты определил тип контакта.',
        };
      ...
      default:
        return {
          ...
          handoffToManager: false,
          ignoreMessages: false,
          additionalInstructions: 'Тип контакта пока неизвестен. Общайся стандартно. Если из контекста разговора непонятно, для себя ли клиент ищет офис или для кого-то другого, можешь при удобном моменте ненавязчиво уточнить: "Вы для себя офис подбираете или для клиента?"',
        };
    }
  }
